...
3. **Leer dedup sources**:
   - `data/applications.md` → empresa + rol ya evaluado
   - `data/pipeline.md` → URL ya en pendientes o procesadas
   - `data/applied-jobs.md` → empresa + rol ya aplicado (dedup crítico — no añadir si ya Applied/Interview/Offer)
...
4. **Nivel 1 — Playwright scan** (paralelo en batches de 3-5):
   ...
   h. **GREENHOUSE EXCLUSION**: Si la URL del job listing contiene `greenhouse.io`, registrar en `scan-history.tsv` con status `skipped_greenhouse` y NO añadir al pipeline. Greenhouse está excluido del workflow automatizado.

5. **Nivel 2 — Greenhouse APIs** (DESHABILITADO — Greenhouse excluido del workflow):
   Greenhouse está excluido del pipeline automatizado. No ejecutar fetch de Greenhouse APIs.
   Si una empresa usa exclusivamente Greenhouse, omitir en Nivel 1 y 2.
...
7. **Deduplicar** contra 4 fuentes:
   ...
   - `applied-jobs.md` → empresa + rol ya aplicado (status Applied/Interview/Offer) → skipped_dup
...
12. **Ofertas en Greenhouse**: registrar con status `skipped_greenhouse`
